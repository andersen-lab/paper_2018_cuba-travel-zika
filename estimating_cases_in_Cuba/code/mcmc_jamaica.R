...
initial_y = logit(c(0.0001, 0.0002, 0.008, 0.003, 0.0002, 0.0002, 0.0001, 0.0001))
...
pdf('../output/jamaica_case_rate.pdf', width = 6, height = 5)
layout(matrix(1:2, 2, 1))
par(mar = c(1, 2, 0.5, 0.5), oma = c(2,1,0,0))
plot(dat$travel.cases[which(dat$country == 'Jamaica')] / dat$travel.pop[which(dat$country == 'Jamaica')], type = 'l',
     col = rgb(1,0,0,0.75), lwd = 3, xaxt = 'n', ylim = c(0, 0.002))
lines(dat$local.cases[which(dat$country == 'Jamaica')] / dat$local.pop[which(dat$country == 'Jamaica')], type = 'l',
...
pdf('../output/accepatance_rate.pdf', width = 6, height = 3)
